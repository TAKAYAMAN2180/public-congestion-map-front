...
const Storepage = () => {
  ...
  const [deductionsSummary, setDeductionSummary] =
    useState<DeductionsSummaryType>({
      attentionTimes: 0,
      deductionPoints: 0,
    });
      ...
      if (cookies) {
        if (cookies.bkc_fes_storeIndex) {
          ...
          try {
            ...
            const getCookieData =
              cookies.bkc_fes_storeIndex as StoreIndexCookieKeptType;
          ...
          if (cookies.bkc_fes_databaseValue) {
            try {
              const getCookieData =
                cookies.bkc_fes_databaseValue as DatabaseValueCookieKeptType;
        ...
        if (cookies.bkc_fes_storeIndex) {
          ...
          if (getCookieStoreIndexData.storeID == storeIDByParam) {
            ...
            if (cookies.bkc_fes_databaseValue) {
              ...
              setIsPageLoading(true);
              if (tempStoreID && tempStoreInfo) {
                await fetchInfosFromDatabase(
                  tempStoreID,
                  tempStoreInfo.areaNum,
                );
              }
              setIsPageLoading(false);
            }
  ...
  const fetchInfosFromDatabase = async (storeID: string, areaNum: number) => {
    const targetStoreInfo = initStoresInfoData.find(
      (element) => element.areaNum == areaNum,
    );
    if (targetStoreInfo) {
      ...
      for (const currentDeduction of getDeductions) {
        ...
        if (targetDeduction) {
          const element: DeductionInfoType = {
            status: currentDeduction.status,
            ...targetDeduction,
          };
          ...
          if (currentDeduction.status == DeductionStatusEnum.ATTENTION) {
            tempDeductionSummary.attentionTimes =
              tempDeductionSummary.attentionTimes + 1;
          } else if (currentDeduction.status == DeductionStatusEnum.DEDUCTION) {
            if (targetDeduction) {
              tempDeductionSummary.deductionPoints =
                tempDeductionSummary.deductionPoints + targetDeduction.points;
            }
          }
        }
      }
      ...
      setCookie(
        ...
        JSON.stringify(storeInfoCookieKeptObj),
        {
          expires: tempTime,
          path: "/",
        },
    ...
    if (storeID == "") {
      ...
      if (pattern.test(storeID)) {
        ...
        try {
          ...
          errorStr =
            "店舗キーがデータベースに登録されていないか、データベースを読込むことができません";
          console.log(error);
        }
        setIsPageLoading(false);
      } else {
        errorStr =
          "店舗IDはアルファベットと数字しか含まれません。確認してください。";
      }
    }
    if (isValid) {
      if (getAreaNum) {
  ...
  const removeQueryParam = (key: string) => {
    ...
    router.push(
      ...
      undefined,
      { shallow: true },
    ); // shallow: true でページ全体の再レンダリングを回避
  };
        ...
        {storeInfo ? (
          <div style={{ padding: "20px" }}>
            <div style={{ fontWeight: "bold" }}>
              <span style={{ fontSize: "2rem" }}>{storeInfo?.storeName}</span>
              <br />
              {storeInfo?.groupName && (
            ...
            <div style={{ marginTop: "30px" }}>
              <div style={{ fontSize: "1.7rem", fontWeight: "bold" }}>詳細</div>
              <InfoBox title={"販売品目"}>{storeInfo?.food}</InfoBox>
              <InfoBox title={"現在の混雑状況"}>
                {congestion &&
                  getCongestionStatusStr(congestion.congestionLevel)}
              </InfoBox>
              <InfoBox title={"減点状況"}>
                <div style={{ fontSize: "1.5rem" }}>
                  注意 {deductionsSummary.attentionTimes}回, 減点{" "}
                  {deductionsSummary.deductionPoints}点
                </div>
                <div>
                  {deductionsInfo.map((deduction, key) => (
                    <div key={key}>
                      {deduction.points}点 減点項目{" "}
                      {getDeductionStatusStr(deduction.status)}
                      <br />
                      {deduction.content}
                    </div>
                  ))}
