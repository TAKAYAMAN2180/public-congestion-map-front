...
export const fetchStoreDeductionsByStoreID = async (storeID: string) => {
  const response = (await API.graphql(
    graphqlOperation(Queries.fetchStoreDeductionsByStoreID, {
      storeID: storeID,
    }),
  )) as {
...
export const fetchStoreCongestionByStoreID = async (storeID: string) => {
  const response = (await API.graphql(
    graphqlOperation(Queries.fetchStoreCongestionByStoreID, {
      storeID: storeID,
    }),
  )) as {
...
export const fetchStoreByStoreID = async (storeID: string) => {
  const response = (await API.graphql(
    graphqlOperation(Queries.fetchStoreByStoreID, {
      storeID: storeID,
    }),
  )) as {
